...
def d4parse(list[str] data) -> list[str]:
    """
    Add bounds for simpler checks
    surround puzzles with dots
    """
    cdef list result = ['.' * (len(data[0]) + 2) ]
    for line in data:
...
def d4p1(list[str] data) -> int:
    """
    part 1
    """
    cdef int size_y = len(data)
    cdef int size_x = len(data[0])
    cdef int result = 0
